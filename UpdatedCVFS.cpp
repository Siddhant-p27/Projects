...
void man(char *name)
{
    ...
    {
        printf("Description: Used to open exhisting file\n");
    ...
    else if((strcmp(name,"closeall") == 0))
    {
        ...
        printf("Usage: closeall File_name mode\n");
    }
...
void DisplayHelp()
{
    ...
    printf("read: To read contents intp file\n");
...
int ReadFile(int fd, char *arr,int isize)
{
    ...
    if(UFDTArr[fd].ptrfiletable->ptrinode->permission != READ+WRITE) return -2;
    ...
    while(1)
    {
        ...
        if(count == 1)
        {
            ...
            else if(strcmp(command[0],"exit") == 0)
            {
                ...
                while( i < MAXINODE)
                {
                    fd = creat(UFDTArr[i].ptrfiletable->ptrinode->FileName,UFDTArr[i].ptrfiletable->mode);
                    read(fd,UFDTArr[i].ptrfiletable->ptrinode->Buffer,UFDTArr[i].ptrfiletable->ptrinode->FileActualSize);
                    write(fd,UFDTArr[i].ptrfiletable->ptrinode->Buffer,UFDTArr[i].ptrfiletable->ptrinode->FileActualSize);  
        ...
        else if(count == 3)
        {
            if(strcmp(command[0],"create") == 0)
            {
                ...
                if(ret>=0)
                    printf("File is successfully with file descriptor: %d\n",ret);
                if(ret == -1)
                    printf("ERROR : Incorrect parametersS\n");
                if(ret == -2)
